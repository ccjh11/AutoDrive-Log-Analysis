...
graph TD
    ...
    subgraph GUI皮囊层
        G[gui_main.py Frontend] <-->|用户交互/多线程回调| B
    end
    
    B --> C{适配解析管线}
    C -->|老式标准宽表| D[data_processor.py]
    C -->|CAN原始长表/故障数据| E[can_parser.py 中台]
...
sequenceDiagram
    ...
    participant Parser as can_parser
    participant Engine as rule_engine
    participant Gen as report_generator

    User->>GUI: 点击【开始分析并生成报告】
    GUI->>GUI: 启动后台子线程 analysis_task
    GUI->>Main: 调用 analyze_log_and_generate_report()
    Main->>Parser: 读入原始长表数据并执行解析
    Parser-->>Main: 返回物理值宽表 DataFrame
    Main->>Engine: 初始化 RuleEngine 并执行 analyze()
    Note over Engine: 激活HIL故障诊断算法<br/>(差分跳变捕获/NaN超时识别)
    Engine-->>Main: 返回诊断出的异常报告列表
    Main->>Gen: 传入变量进行数据渲染
    Gen->>Gen: 落盘输出 report.html & report.docx
    Main->>GUI: 回调通知 on_analysis_done()
    GUI->>GUI: 进度条秒变 100%, 弹出成功弹窗
    GUI-->>User: 展现报告生成成功喜报
```
...
├── output/                     # 📄 输出渲染层
│   └── report_generator.py     # 报告生成器（基于 DataFrame 动态导出 Word/HTML）
├── tests/                      # 🚀 单元测试护航层
